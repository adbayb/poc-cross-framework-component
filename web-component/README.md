...
By using a framework, the developer experience can be greatly improved while letting the framework manages non trivial tasks (performance diffing, rendering lifecycle, server side rendering, ...).  
...
For React developers, Preact with [preact/compat](https://www.npmjs.com/package/@preact/compat) seems to be the ideal candidate to manage web component compatibility layer while providing **a tiny runtime** and **preserving the same good developer experience**.
